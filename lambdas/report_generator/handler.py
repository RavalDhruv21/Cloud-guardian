...
def get_weekly_anomalies():
    """Fetch all anomalies from the past 7 days"""
...
def get_cost_suggestions():
    """Fetch pending cost suggestions"""
    dynamodb = boto3.resource('dynamodb', region_name='us-east-1')
    table = dynamodb.Table(
...
def generate_report_with_ai(anomalies, cost_suggestions):
...
{json.dumps(cost_suggestions, indent=2, default=str)}
...
def save_report_to_s3(report_text):
...
def send_report_via_sns(report_text):
    ...
    sns.publish(
        TopicArn=sns_arn,
        Subject=f"Cloud Guardian Weekly Report — {datetime.now(timezone.utc).strftime('%Y-%m-%d')}",
        Message=report_text
    )
